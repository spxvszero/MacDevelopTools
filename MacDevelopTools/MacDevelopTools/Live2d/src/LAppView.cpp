...
void LAppView::OnTouchesEnded(float px, float py) const
{
    ...
    live2DManager->OnDrag(0.0f, 0.0f);
    {
        ...
        LAppPal::PrintLog("[APP]Size view left:%.2f/%.2f right:%.2f/%.2f top:%.2f/%.2f bottom:%.2f/%.2f scaleX/Y:%.2f/%.2f scaleMin/Max:%.2f/%.2f",
                          _viewMatrix->GetScreenLeft(),_viewMatrix->GetMaxLeft(),
                          _viewMatrix->GetScreenRight(),_viewMatrix->GetMaxRight(),
                          _viewMatrix->GetScreenTop(),_viewMatrix->GetMaxTop(),
                          _viewMatrix->GetScreenBottom(),_viewMatrix->GetMaxBottom(),
                          _viewMatrix->GetScaleX(),_viewMatrix->GetScaleY(),
                          _viewMatrix->GetMinScale(),_viewMatrix->GetMaxScale()
                          );
...
float LAppView::GetTouchX() const
{
    return _touchManager->GetX();
}
float LAppView::GetTouchY() const
{
    return _touchManager->GetY();
}
