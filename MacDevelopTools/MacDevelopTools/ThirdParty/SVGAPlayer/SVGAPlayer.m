...
@property (nonatomic, assign) NSInteger displayLinkInterval;
@property (nonatomic, assign) NSInteger currentFrame;
@property (nonatomic, copy) NSArray *contentLayers;
@property (nonatomic, copy) NSDictionary<NSString *, NSImage *> *dynamicObjects;
...
{
    self.displayLinkInterval = 60 / self.videoItem.FPS;
...
{
    return [(__bridge SVGAPlayer *)displayLinkContext renderTime:*inOutputTime];
}
...
- (CVReturn)renderTime:(CVTimeStamp)inOutputTime
{
    self.count++;
    if (self.count > self.displayLinkInterval) {
        self.count = 0;
...
- (void)initPlayer {
    self.contentMode = SVGAContentModeTop;
...
//                CGSize bitmapSize = CGSizeMake(self.videoItem.images[sprite.imageKey].size.width * self.videoItem.images[sprite.imageKey].scale, self.videoItem.images[sprite.imageKey].size.height * self.videoItem.images[sprite.imageKey].scale);
                CGSize bitmapSize = CGSizeMake(self.videoItem.images[sprite.imageKey].size.width * 1, self.videoItem.images[sprite.imageKey].size.height * 1);
...
- (void)resize {
    if (self.contentMode == SVGAContentModeScaleAspectFit) {
    ...
    else if (self.contentMode == SVGAContentModeScaleAspectFill) {
        CGFloat videoRatio = self.videoItem.videoSize.width / self.videoItem.videoSize.height;
        CGFloat layerRatio = self.bounds.size.width / self.bounds.size.height;
        if (videoRatio < layerRatio) {
            CGFloat ratio = self.bounds.size.width / self.videoItem.videoSize.width;
    ...
    else if (self.contentMode == SVGAContentModeTop) {
    ...
    else if (self.contentMode == SVGAContentModeBottom) {
        CGFloat scaleX = self.frame.size.width / self.videoItem.videoSize.width;
        CGPoint offset = CGPointMake(
    ...
    else if (self.contentMode == SVGAContentModeLeft) {
    ...
    else if (self.contentMode == SVGAContentModeRight) {
        CGFloat scaleY = self.frame.size.height / self.videoItem.videoSize.height;
        CGPoint offset = CGPointMake(
...
- (void)setImage:(NSImage *)image forKey:(NSString *)aKey {
    if (image == nil) {
        return;
    }
    ...
    if (self.contentLayers.count > 0) {
        ...
        //        [trans translateXBy:1 yBy:-1];
                ...
                layer.bitmapLayer.contents = (__bridge id _Nullable)([image CGImageForProposedRect:nil context:nil hints:@{NSImageHintCTM:trans}]);
            }
        }
    }
}
...
- (void)setImage:(NSImage *)image forKey:(NSString *)aKey referenceLayer:(CALayer *)referenceLayer {
    [self setImage:image forKey:aKey];
}
...
//        CGSize bitmapSize = CGSizeMake(self.videoItem.images[aKey].size.width * self.videoItem.images[aKey].scale, self.videoItem.images[aKey].size.height * self.videoItem.images[aKey].scale);
        CGSize bitmapSize = CGSizeMake(self.videoItem.images[aKey].size.width * 1, self.videoItem.images[aKey].size.height * 1);
